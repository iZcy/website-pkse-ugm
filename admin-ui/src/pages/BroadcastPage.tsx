...
export default function BroadcastPage() {
  ...
  const departments = [...new Set(_contacts.map((c: any) => c.department).filter(Boolean))]
  const filtered = _contacts.filter((c: any) => {
      ...
      {step === 1 && (
        <div>
          <div className="flex gap-3 mb-3 flex-wrap">
            ...
            <span className="flex-1 text-sm text-slate-500 self-center text-right">{selected.size} dari {_contacts.length}</span>
          </div>
