...
import { pengumumanMassUploadConfig } from '../lib/massUploadConfigs'
...
export default function PengumumanPage() {
  ...
  return (
    <div>
      ...
      {showMassUpload && <MassUpload config={pengumumanMassUploadConfig} onClose={() => setShowMassUpload(false)} onSuccess={load} />}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <h3 className="text-lg font-bold text-slate-800 p-6 pb-0 flex-shrink-0">{editId ? 'Edit' : 'Tambah'} Pengumuman</h3>
