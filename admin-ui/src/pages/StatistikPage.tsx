...
export default function StatistikPage() {
  ...
  const load = useCallback(async () => {
    try { const d = await apiGet(`/api/cms/stats?period=${period}`); setItems(d.items || d || []) } catch { setItems([]) }
    setLoading(false)
  }, [period])
  useEffect(() => { load() }, [load])
  ...
  async function syncTemplate() { if (!confirm('Sync dari template global?')) return; await apiPost(`/api/cms/sync-stats?period=${period}`, {}); load() }
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <h3 className="text-lg font-bold mb-4">{editId ? 'Edit' : 'Tambah'} Statistik</h3>
