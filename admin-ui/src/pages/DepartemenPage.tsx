  ...
  const load = useCallback(async () => {
    try {
      const [d, m] = await Promise.all([
        ...
        apiGet(`/api/cms/members?period=${period}&per_page=300`),
      ])
      setDepts((d || []).sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0)))
      setMembers((m.items || m || []).sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0)))
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-2 min-w-0">
            ...
            <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center overflow-hidden flex-shrink-0">
              {d.icon_url ? <img src={d.icon_url} className="w-full h-full object-contain p-1" alt="" /> : <span className="text-xl">{d.icon || <Network className="w-5 h-5 text-slate-400" />}</span>}
            </div>
        ...
        <div className="bg-slate-50 rounded-lg p-2 min-h-[36px]">
          {members.length === 0 ? <div className="text-xs text-slate-400 p-1">Belum ada anggota di kementerian ini.</div> : (
            <div className="flex flex-wrap gap-1">{members.map((m: any) => (
              <span key={m.id} className="inline-flex items-center gap-1 text-xs bg-white border border-slate-200 rounded-full px-2 py-1">{m.photo_url ? <img src={m.photo_url} className="w-4 h-4 rounded-full" alt="" /> : null}{m.full_name}</span>
            ))}</div>
          )}
        </div>
      </div>
