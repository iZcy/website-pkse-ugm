...
export default function DepartemenPage() {
  ...
  async function moveItem(id: string, dir: number) {
    const idx = items.findIndex(d => d.id === id)
    const other = items[idx + dir]
    if (!other) return
    try {
      await apiPut(`/api/cms/departments/${id}`, { ...items[idx], sort_order: other.sort_order })
      await apiPut(`/api/cms/departments/${other.id}`, { ...other, sort_order: items[idx].sort_order })
      load()
    } catch (e: any) { alert(e.message) }
  }
        ...
        {items.map((d: any) => (
          <div key={d.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => moveItem(d.id, -1)} className="p-1 rounded hover:bg-slate-100" title="Naik"><ChevronUp className="w-3.5 h-3.5" /></button>
              <button onClick={() => moveItem(d.id, 1)} className="p-1 rounded hover:bg-slate-100" title="Turun"><ChevronDown className="w-3.5 h-3.5" /></button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
            <h3 className="text-lg font-bold text-slate-800 p-6 pb-0 flex-shrink-0">{editId ? 'Edit' : 'Tambah'} Departemen</h3>
            <div className="overflow-y-auto flex-1 p-6 space-y-3">
              ...
              <div><label className="text-sm font-medium">Icon URL</label><ImageUpload value={icon} onChange={(url) => setIcon(url)} /></div>
