...
{sd&&<div className="absolute top-full left-0 right-0 z-50 bg-white border border-slate-300 rounded-b-lg shadow-lg max-h-48 overflow-y-auto mt-0.5">{rs.map(r=><div key={r.nim} onClick={()=>sel(r)} className="px-4 py-2.5 cursor-pointer hover:bg-blue-50 border-b border-slate-100 last:border-0"><div className="text-sm font-medium text-slate-700">{r.name}</div></div>)}</div>}</div>
<p className="text-xs text-slate-400 mt-1">Anggota: ketik nama untuk mencari. Gunakan NIM sebagai password.</p></div>
...
<button type="submit" disabled={ld} className="w-full bg-blue-700 hover:bg-blue-800 text-white font-semibold py-3 rounded-lg transition">{ld?'Memeriksa...':'Masuk'}</button></form></div></div>)}
