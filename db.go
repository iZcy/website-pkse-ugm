...
// GetDepartmentTree returns departments in hierarchical tree structure
func GetDepartmentTree(periodLabel string) ([]DepartmentNode, error) {
	depts, err := GetDepartments(periodLabel)
	if err != nil {
		return nil, err
	}
	return buildDepartmentTree(depts, nil), nil
}

// DepartmentNode extends Department with children
type DepartmentNode struct {
	Department `bson:",inline" json:",inline"`
	Children   []DepartmentNode `bson:"children,omitempty" json:"children,omitempty"`
}

func buildDepartmentTree(depts []Department, parentID *primitive.ObjectID) []DepartmentNode {
	var nodes []DepartmentNode
	for _, dept := range depts {
		// Check if this department's parent matches the current parentID
		matchesParent := (parentID == nil && dept.ParentID == nil) ||
			(parentID != nil && dept.ParentID != nil && parentID.Hex() == dept.ParentID.Hex())
